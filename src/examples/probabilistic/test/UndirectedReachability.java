...
/**
 * Attempts to find a path in a undirected graph.
 * 
 * @author Yash Dhamija
 * @author Xiang Chen
...
public class UndirectedReachability {
	 ...
	 * 
	 * @param args[0] the number of vertices of the undirected graph
	...
	public static void main(String[] args) {		
		if (args.length < 3) {
			System.out.println("Usage: java UndirectedReachability <number of vertices>\n"
					+ "<source vertex>\n"
					+ "<target vertex>\n"
					+ "<edges>");
			System.exit(0);
		...
		incorrect = probabilistic.examples.UndirectedReachability.findPath(adjacent, source, target) != depthFirstSearch(adjacent, source, target, visited);
	}

	/**
	 * Checks whether the given target vertex is reachable from the given start vertex in the given undirected graph
	 * via vertices that have not been visited yet.
	 * 
	 * @param adjacent adjacent the adjacency list of the undirected graph
	 * @param start the start vertex
	 * @param target the target vertex
	 * @param visited whether a vertex has been visited
	 * @return true if the given target vertex is reachable from the given start vertex in the given undirected graph
